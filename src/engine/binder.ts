...
import type {
  Expr, SelectStmt, SelectLike, SetOpStmt, TableRef, Pos,
} from '../sql/ast';
...
import type {
  LogicalPlan, OutputColumn, BoundExpr, AggregateSpec, WindowSpec,
} from './logical-plan';
...
export class Binder {
  ...
  private windowContexts: WindowSpec[][] = [];
  /** 仅 SELECT 列表允许窗口函数 */
  private allowWindow = false;
  /** 当前绑定的子句名（用于窗口函数位置报错） */
  private windowClause = 'this clause';
  /** 窗口子句内不允许再嵌套窗口 */
  private insideWindow = false;
  /**
   * 窗口子句绑定模式：其中的聚合调用（如 RANK() OVER (ORDER BY SUM(x))）
   * 编译为对聚合输出行 label 的读取（窗口算子在聚合算子之上，行上已有该值）
   */
  private aggRowKeyMode = false;
  ...
  bindSelect(stmt: SelectLike): { plan: LogicalPlan; outputs: OutputColumn[] } {
    if (stmt.kind === 'setOp') return this.bindSetOp(stmt);

    const scope = new Scope();
    this.scopes.push(scope);
    const aggCtx: AggContext = { aggregates: [], hasGroupBy: false, groupExprIds: [] };
    this.aggContexts.push(aggCtx);
    this.windowContexts.push([]);
    const savedAllowWindow = this.allowWindow;
    const savedClause = this.windowClause;
    this.allowWindow = false;
    ...
    // 5. HAVING
    if (stmt.having) {
      this.windowClause = 'HAVING';
      const h = this.bindExpr(stmt.having);
      this.windowClause = savedClause;
      plan = { node: 'filter', input: plan, predicate: h };
    }

    // 5.5 窗口函数：在分组/聚合/HAVING 之后、投影之前计算
    const windowSpecs = this.windowContexts[this.windowContexts.length - 1];
    if (windowSpecs.length > 0) {
      plan = { node: 'window', input: plan, specs: windowSpecs };
    }
    ...
    if (stmt.orderBy.length > 0) {
      ...
      const keys = stmt.orderBy.map((o) => {
        ...
        if (o.expr.kind === 'literal' && typeof o.expr.value === 'number' && Number.isInteger(o.expr.value)) {
          ...
          exprId = this.bindOrderExpr(o.expr, projectItems, outputs, scope, o.pos);
        }
    ...
    this.allowWindow = savedAllowWindow;
    this.windowClause = savedClause;
    return { plan, outputs };
  }
  ...
  // ---------------------------------------------------------
  private bindSetOp(stmt: SetOpStmt): { plan: LogicalPlan; outputs: OutputColumn[] } {
    // 展平左结合树为有序分支序列
    const branches: SelectStmt[] = [];
    const ops: SetOpStmt['op'][] = [];
    const flatten = (s: SelectLike): void => {
      if (s.kind === 'setOp') {
        flatten(s.left);
        ops.push(s.op);
        branches.push(s.right);
      } else {
        branches.push(s);
      }
    };
    ...
    const first = this.bindSelect(branches[0]);
    const outputs = first.outputs;
    const outLabels = outputs.map((o) => o.label);
    let plan = first.plan;

    for (let i = 0; i < ops.length; i++) {
      const bound = this.bindSelect(branches[i + 1]);
      if (bound.outputs.length !== outputs.length) {
        throw new SQLError(
          `each SELECT of a compound query must have the same number of result columns: ` +
          `SELECT #1 has ${outputs.length} but SELECT #${i + 2} has ${bound.outputs.length}`,
          branches[i + 1].pos, 'BIND',
        );
      }
      plan = {
        node: 'setOp',
        ...
        left: plan,
        right: bound.plan,
        leftLabels: outLabels,
        rightLabels: bound.outputs.map((o) => o.label),
      };
    }

    // 复合查询的 ORDER BY：只允许引用输出列名或位置序号
    if (stmt.orderBy.length > 0) {
      const keys = stmt.orderBy.map((o) => {
        let label: string;
        if (o.expr.kind === 'literal' && typeof o.expr.value === 'number' && Number.isInteger(o.expr.value)) {
        ...
        } else if (o.expr.kind === 'column' && !o.expr.table) {
          const lower = o.expr.name.toLowerCase();
          const hit = outputs.find((c) => c.label.toLowerCase() === lower);
          if (!hit) {
            throw new SQLError(
              `ORDER BY "${o.expr.name}" is not an output column of the compound query`,
              o.pos, 'BIND',
            );
          }
          label = hit.label;
          ...
          throw new SQLError(
            'ORDER BY on a compound query must use an output column name or position',
            o.pos, 'BIND',
          );
        }
        const exprId = this.pool.add({
  ...
  // 窗口函数
  // ---------------------------------------------------------
  private bindWindow(e: Extract<Expr, { kind: 'window' }>): number {
    if (!this.allowWindow || this.windowContexts.length === 0) {
      throw new SQLError(
        `window function ${e.func.name}() is not allowed in ${this.windowClause}`,
        e.pos, 'BIND',
      );
    }
    ...
    if (e.func.distinct) {
      throw new SQLError(`DISTINCT is not supported in window function ${e.func.name}()`, e.pos, 'BIND');
    }
    const specs = this.windowContexts[this.windowContexts.length - 1];

    const savedInside = this.insideWindow;
    const savedAggRowKey = this.aggRowKeyMode;
    this.insideWindow = true;
    this.aggRowKeyMode = true;
    let arg: number | null = null;
    let partitionBy: number[] = [];
    let orderBy: { expr: number; desc: boolean; nullsFirst: boolean }[] = [];
    try {
      if (e.func.star) {
        if (e.func.name !== 'COUNT') {
          throw new SQLError(`window function ${e.func.name}(*) is not supported`, e.func.pos, 'BIND');
        }
      } else if (e.func.args.length === 1) {
        arg = this.bindExpr(e.func.args[0]);
      } else if (e.func.args.length > 1) {
        throw new SQLError(`window function ${e.func.name} expects at most 1 argument`, e.func.pos, 'BIND');
      }
      ...
      this.insideWindow = savedInside;
      this.aggRowKeyMode = savedAggRowKey;
    }

    // 防御：窗口子句中的聚合应已被编译为行键读取（??AGGROW），
    // 若出现 frame.agg 形式（??AGG）说明绑定路径有误，直接报错而非静默出错
    const boundIds = [arg, ...partitionBy, ...orderBy.map((o) => o.expr)]
      .filter((id): id is number => id !== null);
    for (const id of boundIds) {
      if (this.pool.get(id).debug.includes('??AGG(')) {
        throw new SQLError('aggregate functions are not supported inside window clauses', e.pos, 'BIND');
      }
    }
    ...
    const dedupe = [
      e.func.name,
      arg === null ? (e.func.star ? '*' : '-') : this.pool.get(arg).debug,
      partitionBy.map((id) => this.pool.get(id).debug).join(','),
      orderBy.map((o) => `${this.pool.get(o.expr).debug}${o.desc ? 'D' : 'A'}${o.nullsFirst ? 'F' : 'L'}`).join(','),
    ].join('|');
    let index = specs.findIndex((s) => s.dedupe === dedupe);
    if (index < 0) {
      index = specs.length;
      specs.push({
        kind: e.func.name as WindowSpec['kind'],
        arg,
        partitionBy,
        orderBy,
        key: `__win_${index}`,
    ...
    return this.pool.add({
      eval: (row) => row[key] ?? null,
      debug: `win(${e.func.name}#${index})`,
      columnRefs: new Set(),
      constant: false,
    });
  }
  ...
  private bindAggregate(e: Extract<Expr, { kind: 'func' }>): number {
    ...
    if (ctx && ctx.allowAgg !== false && !inAggArg) {
      ...
      if (this.aggRowKeyMode) {
        // 窗口子句内的聚合：值已由下方聚合算子物化到行上（以 label 为键）
  ...
  private bindSubqueryPlan(stmt: SelectLike): { plan: LogicalPlan; outputs: OutputColumn[] } {
      ...
      switch (p.node) {
        ...
        case 'window':
          for (const s of p.specs) {
            for (const id of s.partitionBy) check(id);
            for (const o of s.orderBy) check(o.expr);
            if (s.arg !== null) check(s.arg);
          }
          walk(p.input);
          break;
        case 'setOp':
  ...
  defaultLabel(e: Expr): string {
    switch (e.kind) {
      ...
      case 'star': return '*';
      case 'window': return `${e.func.name}() OVER (...)`;
    }
  }
  ...
  private inferOutputType(e: Expr, id: number, ctx: AggContext): OutputColumn['type'] {
    ...
    if (e.kind === 'window') {
      const n = e.func.name;
      if (n === 'ROW_NUMBER' || n === 'RANK' || n === 'COUNT') return 'INTEGER';
      if (n === 'AVG') return 'REAL';
      return 'ANY';
    }
    // 聚合结果类型已知
