...
import type {
  Stmt, SelectStmt, SelectLike, InsertStmt, UpdateStmt, DeleteStmt,
} from '../sql/ast';
...
export class Engine {
  ...
  // SELECT
  // ---------------------------------------------------------
  private prepareSelect(stmt: SelectLike): PreparedSelect {
  ...
  private runSelect(stmt: SelectLike, pos: { line: number; column: number }): QueryResult {
    if (this.txn?.failed) {
      throw new SQLError(
  ...
  private async runWrite(
    stmt: Exclude<Stmt, { kind: 'select' | 'setOp' | 'txn' | 'explain' }>,
    pos: { line: number; column: number },
  ): Promise<QueryResult> {
  ...
  private prepareRowExpr(
    ...
    // UPDATE SET 的主计划无法执行窗口函数（子查询内的窗口不受影响）
    const planHasWindow = (p: LogicalPlan): boolean => {
      if (p.node === 'window') return true;
      if ('input' in p && p.input) return planHasWindow(p.input);
      if (p.node === 'join' || p.node === 'setOp') return planHasWindow(p.left) || planHasWindow(p.right);
      return false;
    };
    if (planHasWindow(bound.plan)) {
      throw new SQLError('window functions are not supported in UPDATE SET expressions', pos, 'BIND');
    }
    const subqueries = new SubqueryRuntime();
  ...
  private async runExplain(inner: Stmt, pos: { line: number; column: number }): Promise<QueryResult> {
    if (inner.kind !== 'select' && inner.kind !== 'setOp') {
...
export function formatLogicalPlan(plan: LogicalPlan, binder: Binder, indent: string): string {
  ...
  switch (plan.node) {
    ...
    case 'distinct':
      ...
      break;
    case 'window': {
      const specText = plan.specs.map((s) => {
        const part = s.partitionBy.map((id) => expr(id)).join(', ');
        const ord = s.orderBy.map((o) => `${expr(o.expr)} ${o.desc ? 'DESC' : 'ASC'}`).join(', ');
        const arg = s.arg === null ? '' : expr(s.arg);
        return `${s.kind}(${arg}) OVER (PARTITION BY ${part}${ord ? ` ORDER BY ${ord}` : ''})`;
      }).join(', ');
      lines.push(`${pad}Window: ${specText}`);
      lines.push(formatLogicalPlan(plan.input, binder, indent + '  '));
      break;
    }
    case 'setOp':
...
export function formatPhysicalPlan(plan: LogicalPlan, binder: Binder, indent: string): string {
  ...
  switch (plan.node) {
    ...
    case 'distinct':
      ...
      break;
    case 'window':
      lines.push(`${pad}WindowAgg(specs=${plan.specs.map((s) => s.kind).join(', ')})`);
      lines.push(formatPhysicalPlan(plan.input, binder, indent + '  '));
      break;
    case 'setOp':
      lines.push(`${pad}${plan.op === 'UNION ALL' ? 'UnionAll' : plan.op === 'UNION' ? 'Union' : 'Except'}`);
      lines.push(formatPhysicalPlan(plan.left, binder, indent + '  '));
      lines.push(formatPhysicalPlan(plan.right, binder, indent + '  '));
      break;
    case 'subquerySource':
