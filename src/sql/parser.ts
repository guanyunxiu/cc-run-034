...
import type {
  Stmt, SelectStmt, SelectLike, SetOpStmt, SetOpKind, SelectItem, TableRef, JoinRef, OrderItem, Expr,
  InsertStmt, UpdateStmt, DeleteStmt, CreateTableStmt, DropTableStmt,
  CreateIndexStmt, DropIndexStmt, FuncCallExpr, Pos,
} from './ast';
...
export class Parser {
  ...
  // ---------------- SELECT ----------------
  /**
   * select := selectCore ( (UNION [ALL|DISTINCT] | EXCEPT) selectCore )*
   * 集合运算左结合；复合查询只允许末尾一个 ORDER BY/LIMIT（作用于整体），
   * 解析时把它从最后一个 core 提升到最外层 setOp 节点。
   */
  private parseSelect(): SelectLike {
    const first = this.parseSelectCore();
    if (!this.isKw('UNION') && !this.isKw('EXCEPT')) return first;

    let left: SelectLike = first;
    for (;;) {
      let op: SetOpKind;
      if (this.consumeKw('UNION')) {
        if (this.consumeKw('ALL')) op = 'UNION ALL';
        else { this.consumeKw('DISTINCT'); op = 'UNION'; }
      } else if (this.consumeKw('EXCEPT')) {
        op = 'EXCEPT';
      } else {
        break;
      }
      const right = this.parseSelectCore();
      left = { kind: 'setOp', op, left, right, orderBy: [], limit: null, offset: null, pos: first.pos };
    }

    const root = left as SetOpStmt;
    // 非末尾的分支不允许自带 ORDER BY/LIMIT
    const checkTail = (s: SelectLike, isLast: boolean): void => {
      if (s.kind === 'setOp') {
        checkTail(s.left, false);
        checkTail(s.right, isLast);
        return;
      }
      if (isLast) return;
      if (s.orderBy.length > 0 || s.limit !== null || s.offset !== null) {
        throw new SQLError(
          'ORDER BY/LIMIT must appear after the last SELECT of a compound query',
          s.orderBy[0]?.pos ?? s.pos, 'SYNTAX',
        );
      }
    };
    checkTail(root, true);
    // 末尾 core 的 ORDER BY/LIMIT 属于整个集合结果
    const last = root.right;
    root.orderBy = last.orderBy; last.orderBy = [];
    root.limit = last.limit; last.limit = null;
    root.offset = last.offset; last.offset = null;
    return root;
  }
  ...
  private parseInsert(): InsertStmt {
    ...
    let select: SelectLike | null = null;
  ...
  /** 只能作为窗口函数使用的名字（ROW_NUMBER/RANK 按标识符词法，此处识别） */
  private isWindowFuncName(upper: string): boolean {
  ...
  private parseIdentOrFunc(): Expr {
    ...
    if (this.is('LPAREN') && (t.kind === 'IDENT' || this.isFuncName(t.upper))) {
      ...
      this.expect('RPAREN');
      const windowOnly = this.isWindowFuncName(name);
      // OVER (...) => 窗口函数
      if (this.consumeKw('OVER')) {
        if (!this.isFuncName(name) && !windowOnly) {
          throw this.errorAt(t, `unknown function "${t.value}" (supported: COUNT/SUM/AVG/MIN/MAX, window: ROW_NUMBER/RANK)`);
        }
        return this.parseWindowFunc({ kind: 'func', name, distinct, args, star, pos }, t);
      }
      if (windowOnly) {
        throw this.errorAt(t, `window function ${name}() requires an OVER clause`);
      }
  ...
  /** OVER 已消费：解析 ( [PARTITION BY ...] [ORDER BY ...] ) */
  private parseWindowFunc(func: Extract<Expr, { kind: 'func' }>, nameTok: Token): Expr {
    if (!this.is('LPAREN')) {
      throw this.error('expected "(" after OVER (named window definitions are not supported)');
    }
    this.next();
    ...
    this.expect('RPAREN');
    if (this.isWindowFuncName(func.name)) {
      if (func.star || func.args.length > 0) {
        throw this.errorAt(nameTok, `window function ${func.name}() takes no arguments`);
      }
      if (func.distinct) {
        throw this.errorAt(nameTok, `window function ${func.name}() takes no arguments`);
      }
    }
    return { kind: 'window', func, partitionBy, orderBy, pos: func.pos };
  }
