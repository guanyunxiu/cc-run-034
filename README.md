...
npm test           # 运行内置测试套件（Node + esbuild，155 个用例）
```
...
- 集合运算：`UNION` / `UNION ALL` / `EXCEPT`（左结合，可嵌套子查询，
  整体可再接 `ORDER BY` / `LIMIT`；两侧列数不一致报错并指出左右）。
  行相等性遵循集合语义：`NULL` 与 `NULL` 视为同一行。
- 窗口函数：`ROW_NUMBER()`、`RANK()`、`SUM/COUNT/AVG/MIN/MAX(...) OVER (...)`，
  支持 `PARTITION BY` 与窗口内 `ORDER BY`（聚合窗口默认帧为
  `RANGE UNBOUNDED PRECEDING .. CURRENT ROW`）；仅允许出现在 SELECT 列表，
  写在 `WHERE`/`GROUP BY`/`HAVING` 中会报错。
...
Binder（名称解析、作用域、相关子查询、聚合重写）
  │
  ▼
Logical Plan（scan/filter/project/join/aggregate/sort/limit/distinct/window/setOp）
  │
  ▼
Optimizer（规则优化）
...
- `test/` — 155 个用例，覆盖增删改查、JOIN、聚合、子查询、NULL、类型转换、
  边界、错误（含行列号）、事务、索引、导入导出、IndexedDB 持久化、
  窗口函数与集合运算。
